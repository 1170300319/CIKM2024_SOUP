...
from util.utils import load_state_dict, set_global_logging_level
...
class TrainerBase(object):
    ...
    def create_config(self):
        ...
        config = config_class.from_pretrained(self.args.path)
    ...
    def create_model(self, model_class, config=None, **kwargs):
        ...
        model_name = self.args.path

        model = model_class.from_pretrained(
            model_name,  # t5
            config=config,  # t5load出来的config
            **kwargs
        )
        return model

    def create_tokenizer(self, **kwargs):
        from src.models.tokenization import P5Tokenizer

        if 'p5' in self.args.tokenizer:
            tokenizer_class = P5Tokenizer

        # tokenizer_name = self.args.backbone
        tokenizer_name = self.args.path
