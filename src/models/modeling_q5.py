...
class JointEncoder(T5Stack):
    def __init__(self, config, embed_tokens=None):
        ...
        ## Set maximum 102, 0~100 for indexing time information, 101 for other tokens
        self.time_embeddings = nn.Embedding(
            102, config.d_model   ## config.d_model is 768 for base
        )
        self.init_weights()
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if time_ids is not None:
                ...
                assert time_embeds.shape[-1] == inputs_embeds.shape[-1]
...
class Q5(T5ForConditionalGeneration):
...
@dataclass
class Q5Seq2SeqLMOutput(ModelOutput):
    """
